...
fn main() {
    ...
    for (id, line) in input.lines().map(|line| line.trim()).enumerate() {
        let g = get_game_from_line(id as i32, line);
        all_games.insert(g.id, g);
    }

    let mut accum = 0;
    let mut power_accum = 0;
    for (id, game) in all_games {
        if game.possible {
            accum = accum + id;
        }
        power_accum = power_accum + (game.red * game.blue * game.green);
    }
    println!("accum value={}, power_accum={}", accum, power_accum);
}

fn get_game_from_line(id: i32, line: &str) -> Game {
    let mut g = Game{id:id + 1, red: 0, blue: 0, green: 0, possible: true};
...
mod tests {
    ...
    fn test_getting_game_from_line() {
        ...
        for (test_num, test) in tests.into_iter().enumerate() {
            assert_eq!(get_game_from_line(test_num as i32, test.0), test.1);
        }
    }
}
